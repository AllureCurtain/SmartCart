...
1. **页面骨架极简克制**——中性 off-white 底、黑白为骨、hairline 分隔，信息层级全靠字号与字重。商品和价格是主角，不与任何装饰竞争。
...
| 组 | 关键值 | 说明 |
|----|--------|------|
| 骨架 | `bg #FAFAFA` `ink #111418` `sub #555A61` `meta #6B7178` `hairline #ECECEC` | 中性 off-white 底 + 近黑墨色，sub/meta 均过 WCAG AA 4.5:1 |
...
| 字号 | display 32 · title 20 · price 22 · item 15 · body 16 · label 14 · micro 12 · caption 11 · term 13.5 | 价格与标题靠字重(800)分层 |
| 圆角 | xs 4 · sm 8 · md 12 · lg 14 | 终端/卡片 md，输入框/主按钮 lg，徽标 xs |
| fontVariant | `tabular: ['tabular-nums']` | 价格、百分比等需要对齐的数字 |
| 间距 | xs 4 · s 8 · m 12 · l 16 · xl 20 · xxl 24 · xxxl 32 · pageTop 56 · touchTarget 44 | 4pt 基尺，pageTop 为安全区+标题留白 |

## 核心组件规范

### AGENT TRACE 终端窗口（搜索页）
- 深色块 `termBg`，圆角 md，标题行 `AGENT TRACE` + 端到端耗时 `⏱ 12.3s`（mono、dim）
